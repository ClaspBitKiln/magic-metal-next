...
def api(path, key):
    ...
    with urllib.request.urlopen(req, timeout=45) as r:
        ...
        if isinstance(d, dict) and d.get("success") is False:
            raise RuntimeError(d.get("message","KomTender error"))
        return d
...
def tons(p):
    q, u = p.get("quantity"), text(p.get("unit"))
    if q is not None and ("т" in u or "тон" in u): return float(q)
    import re
    m = re.search(r"(\d+(?:[.,]\d+)?)\s*(?:т|тн|тонн|тонны|тонна)\b", text(p.get("name")))
...
def excel_xml(rows):
    ...
    for row in rows: out.append("<Row>"+"".join(cell(row.get(c,"")) for c in cols)+"</Row>")
...
def main():
    ...
    for page in range(1, pages+1):
        ...
        for short in data[:args.scan]:
            ...
            seen.add(tid)
            try:
                detail = api(urllib.parse.quote(tid), key); calls += 1
            except Exception as e:
                print("skip", tid, e, file=sys.stderr); calls += 1; continue
            ...
            customer, positions, price = detail.get("customer") or {}, detail.get("positions") or [], detail.get("price") or {}
