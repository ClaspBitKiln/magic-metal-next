    ...
    <main className="materials-page">
      ...
      <section className="materials-directory">
        {materialGroups.map((group, groupIndex) => {
          ...
          return <details className="materials-group" id={group.key} open={groupIndex === 0} key={group.key}>
            <summary className="materials-group-title"><span>{String(groupIndex + 1).padStart(2, '0')}</span><h2>{group.label}</h2><p>{groupMaterials.length} {groupMaterials.length === 1 ? 'материал' : 'материалов'}</p><b aria-hidden="true" /></summary>
