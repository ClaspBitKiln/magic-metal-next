...
export default async function PipeDetailPage({ params }: { params: Promise<{ slug: string; detail: string }> }) {
  ...
  return <main className="pipe-detail">
    ...
    <section className="pipe-request"><div><p>Для точного предложения</p><h2>Что указать в заявке</h2></div><ol>{item.requestFields.map((value, index) => <li key={value}><b>{String(index + 1).padStart(2, '0')}</b><span className="standard-code">{value}</span></li>)}</ol></section>
