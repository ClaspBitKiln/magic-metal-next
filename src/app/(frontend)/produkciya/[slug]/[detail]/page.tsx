...
export default async function PipeDetailPage({ params }: { params: Promise<{ slug: string; detail: string }> }) {
  ...
  return <main className="pipe-detail">
    ...
    <section className="pipe-spec"><div><p>Продукция</p><h2>Основные параметры</h2></div><dl>{item.range.map((row) => <div key={row.label}><dt>{row.label}</dt><dd>{row.value}</dd></div>)}</dl></section>
    <section className="pipe-facts"><div className="pipe-facts-heading"><p>Сводные характеристики</p><h2>Поставка и применение</h2></div><dl><div><dt>Марки и материалы</dt><dd>{item.grades.map((value) => <span key={value}>{value}</span>)}</dd></div><div><dt>Применение</dt><dd>{item.applications.map((value) => <span key={value}>{value}</span>)}</dd></div></dl></section>
    <section className="pipe-standards"><div><p>Документы</p><h2>Стандарты</h2></div><div>{item.standards.map((value) => { const standard = findStandard(value); return standard ? <Link href={`/spravochnik-gost/${standard.slug}`} key={value}><span className="standard-code">{value}</span><small>{standard.status}</small><b>↗</b></Link> : <div key={value}><span className="standard-code">{value}</span><small>По проекту</small></div> })}</div></section>
