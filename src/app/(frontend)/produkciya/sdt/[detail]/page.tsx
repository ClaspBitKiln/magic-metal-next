...
export default async function SdtDetailPage({ params }: { params: Promise<{ detail: string }> }) {
  ...
  return (
    <main className="sdt-detail">
      ...
      <section className="sdt-request"><div><p>Что указать в заявке</p><h2>Данные для точного расчёта</h2></div><ol>{item.requestFields.map((value, index) => <li key={value}><b>{String(index + 1).padStart(2, '0')}</b><span className="standard-code">{value}</span></li>)}</ol></section>
