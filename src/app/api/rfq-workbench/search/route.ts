...
export async function POST(request: Request) {
  ...
  const adapter = process.env.RFQ_WORKBENCH_DEMO === 'true' || !process.env.FIRECRAWL_API_KEY ? demoAdapter : metalinfoAdapter

  try {
    const result = await runProcurement(text, [adapter])
    return NextResponse.json({ ...result, sourceMode: adapter === demoAdapter ? 'demo' : 'live' })
  } catch (error) {
