...
export default function QuoteCart() {
  ...
  return <div className="quote-layout">
    ...
    <form className="quote-form" onSubmit={submit}><p>Получить КП</p><h2>Куда отправить предложение?</h2><label>Имя<input name="name" autoComplete="name" /></label><label>Компания<input name="company" autoComplete="organization" /></label><label>Телефон<input name="phone" type="tel" autoComplete="tel" /></label><label>Email<input name="email" type="email" autoComplete="email" /></label><label>Комментарий<textarea name="comment" rows={4} placeholder="Адрес доставки, желаемый срок, особые требования" /></label><input className="quote-honeypot" name="website" tabIndex={-1} autoComplete="off" /><button disabled={status === 'sending'}>{status === 'sending' ? 'Отправляем…' : 'Запросить КП ↗'}</button>{error && <span className="quote-error">{error}</span>}<small>Цена, фактический остаток и срок поставки подтверждаются в КП.</small></form>
  </div>
}
