  ...
  async function runSearch() {
    ...
    try {
      ...
      setStatusMessage(
        ...
        '. Каждое требует проверки менеджером перед выбором закупки.',
      )
    } catch (error) {
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#f4f6f8', color: '#16202a', fontFamily: 'Arial, sans-serif' }}>
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: 28 }}>
        <div style={{ display: 'flex', gap: 8, marginBottom: 18 }}>
          {['request', 'search'].map((stage, index) => (
            <div key={stage} style={{ flex: 1, padding: 10, borderRadius: 8, background: step === stage ? '#0f6b72' : '#e5eaee', color: step === stage ? '#fff' : '#52616d', fontSize: 13 }}>
              {index + 1}. {stage === 'request' ? 'Заявка' : 'Поиск и выбор'}
            </div>
          ))}
        </div>
        ...
        {step === 'search' && (
          <section style={card}>
            {data?.sourceMode === 'demo' && <div style={demoNotice}>Тестовый режим: показаны демонстрационные предложения. Для живого поиска требуется FIRECRAWL_API_KEY.</div>}
...
const demoNotice: React.CSSProperties = { marginBottom: 18, padding: 14, background: '#fff4d6', border: '1px solid #f0d489', borderRadius: 9, lineHeight: 1.5 }
