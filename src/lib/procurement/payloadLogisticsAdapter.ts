...
export async function loadLogisticsRoutes(
  ...
  const result = await payload.find({
    ...
    where: { and: [{ destination: { equals: destination } }, ...(options.tonnage ? [{ tonnage: { equals: options.tonnage } }] : [])] },
    limit: 1000,
    sort: '-dateFrom',
    depth: 0,
  })
  ...
  return [...latest.values()]
    ...
    .map((doc) => ({
      id: String(doc.routeId), origin: String(doc.origin), destination: String(doc.destination),
      mode: doc.mode as LogisticsRoute['mode'], fixedCost: Number(doc.averagePriceRub), currency: 'RUB',
      observedAt: new Date(String(doc.observedAt)).toISOString(), confidence: doc.evidenceLevel === 'verified' ? 0.95 : 0.6,
