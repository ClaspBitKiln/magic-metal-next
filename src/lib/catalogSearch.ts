...
const structuredRules: Record<string, StructuredRule> = {
  ...
  'лист г/к': (row) => inCategory(row, 'Листовой прокат') && /СТАЛЬ ЛИСТ(?:ОВАЯ)? Г\/К/.test(row.product),
  'лист х/к': (row) => inCategory(row, 'Листовой прокат') && productIs(row, 'СТАЛЬ ЛИСТОВАЯ Х/К'),
